...
async def stream_pipeline_job(job_id: str):
    ...
    async def sse_event_generator():
        ...
        try:
            ...
            crop_url = None
            for frame in frames:
                crops_info = reid_matcher.embedder.detect_and_crop(frame, conf_threshold=0.15)
                if crops_info:
                    crops_info[0][0].save(crop_path)
                    crop_url = f"/outputs/pipeline/{inc_id}_crop.jpg"
                    break

            if not crop_url and len(frames) > 0:
                mid_frame = frames[len(frames) // 2]
                h, w = mid_frame.shape[:2]
                roi = mid_frame[int(h * 0.15):int(h * 0.85), int(w * 0.25):int(w * 0.75)]
                roi_rgb = cv2.cvtColor(roi, cv2.COLOR_BGR2RGB)
                Image.fromarray(roi_rgb).save(crop_path)
                crop_url = f"/outputs/pipeline/{inc_id}_crop.jpg"
